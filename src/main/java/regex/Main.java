...
public class Main {
    ...
    public static boolean checkForPassword(String str, int minLength) {
        ...
        final boolean hasLowerCase = Pattern.compile("[a-z]").matcher(str).find();
        final boolean hasUpperCase = Pattern.compile("[A-Z]").matcher(str).find();
        final boolean hasDigit = Pattern.compile("\\d").matcher(str).find();
        final boolean meetsMinLength = str.length() >= minLength;
        return hasLowerCase && hasUpperCase && hasDigit && meetsMinLength;
    }
    ...
    public static boolean checkForDoubles(String str) {
        ...
        return str.matches(".*(.).*\\1.*") && Pattern.compile("([A-Z]).*\\1").matcher(str).find();
    }
}
